...
**Backend:** the `scan_runs` table already exists from Step 2 — it was dead code (no INSERT path wired in) but the schema was richer than what this section originally specced. Reuse it: add a single `kind` column (CHECK on `full | rescan_unmatched | rematch_for_series`, accepting all three even though `rematch_for_series` has no INSERT path; cheap forward-compat). Extend `scan_run_repo` with `complete_with_stats(id, ScanCompletion)` for the success path and `mark_running_as_failed(error_msg)` for the startup sweep. Scanner inserts a `running` row at the start of `scan_full` / `rescan_unmatched`, marks it `completed` (with final stats) or `failed` on exit. `rematch_for_series` is intentionally not persisted (internal plumbing — auto-rematches fire from add-series / refresh / match-from-cv / match-folder-from-cv; the mid-scan pill covers in-flight visibility, tracing covers failure debugging). On server startup (main.rs, after pool/migrations, before `axum::serve`), sweep: any `running` rows get marked `failed` with `error_message = "interrupted by restart"`.

Reusing `scan_runs` (vs. the original `scan_history` proposal) keeps `files_added` and `files_updated` as separate columns from `files_matched` — exactly the delta visibility this section's spec called out as a likely fast follow-up. We get it for free.

**Frontend:** dashboard "last scan" reads the most recent `completed` row from `scanStatus.recent`. `/scans` page reads `list_recent(10)` filtered to user-triggered kinds (Full + RescanUnmatched). In-memory `ScanStatus` keeps only `current` for the live mid-scan pill; the `recent` ring is removed. `ScanRun` (formerly `ScanReport`) is the wire shape — loses per-file errors and `files_marked_missing` (logs cover the former, low-value the latter). API: `GET /api/scans/recent` returns `Vec<ScanRunRow>`.
