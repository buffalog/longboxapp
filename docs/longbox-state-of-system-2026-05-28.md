...
- **Live-DB mutation discipline — never `docker cp` for SQLite mutations** (codified 2026-05-29 after the 6c.3 cycle-resume WAL clobber). SQLite's WAL holds recently-committed-but-not-yet-checkpointed pages relative to the main file; any host-side mutation of `longbox.db` without coordinating WAL+SHM is a coin-flip between data loss (WAL pages silently dropped on restore) and corruption (WAL pages replayed against a modified main). **Forbidden:** `docker cp longbox:/data/longbox.db /tmp/x.db && sqlite3 /tmp/x.db '...' && docker cp /tmp/x.db longbox:/data/longbox.db`. **Two safe paths:** (1) API path — use existing endpoints (`PATCH /api/pull-list/:id`, `PUT /api/downloader`, etc.); add an endpoint before mutating if none exists. (2) In-container sidecar: `docker run --rm --volumes-from longbox alpine sh -c "apk add --no-cache sqlite && sqlite3 /data/longbox.db < /sql && chown -R 1000:1000 /data"` — `sqlite3` runs inside the volume, opens the DB in a single connection that exits cleanly so the WAL gets checkpointed properly. Same standing as the `--force-recreate` and `sqlx prepare` rules: it's the rule, not a remembered intention. Full text + recurrence history in `longbox-phase-a9-prompt.md` Workflow rules.
